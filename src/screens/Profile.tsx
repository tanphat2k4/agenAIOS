...
export function Profile() {
  ...
  const profileTabs = [
    ...
    ...t,
    onSelect: () => s.setProfileTab(t.k),
    bg: t.k === s.profileTab ? 'var(--jade-soft)' : 'transparent',
    fg: t.k === s.profileTab ? 'var(--jade-deep)' : 'var(--ink-2)',
    bd: t.k === s.profileTab ? 'var(--jade)' : 'transparent',
    weight: t.k === s.profileTab ? 700 : 500,
  }))
  ...
  const profileStats = [
    { label: 'Phòng quản lý', value: '8', icon: '🏠' },
    { label: 'Agent sở hữu', value: '18', icon: '🤖' },
    { label: 'Workflow', value: '5', icon: '⚡' },
    { label: 'Thành viên', value: '8', icon: '👥' },
  ]
  ...
  const twoFALabel = s.twoFA ? 'Đang bật' : 'Đang tắt'
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <header style={{ flex: 'none', background: 'var(--surface)', borderBottom: '1px solid var(--line)', padding: '16px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginBottom: 3 }}>Workspace › Tài khoản</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <span style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-.4px' }}>Tài khoản của tôi</span>
            <span style={{ fontSize: 12, color: 'var(--ink-2)' }}>Quản lý hồ sơ, bảo mật và phiên đăng nhập</span>
          </div>
        </div>
        <Hover
          as="button"
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px 36px' }}>
        <div style={{ maxWidth: 860 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 20, overflow: 'hidden', marginBottom: 18 }}>
            {/* gradient cover */}
            <div style={{ height: 91, background: 'linear-gradient(120deg,#28409E,#3B5BDB 60%,#0E7490)', width: 864 }} />
            {/* avatar + info row */}
            <div style={{ padding: '0 24px 22px', display: 'flex', alignItems: 'flex-end', gap: 18, marginTop: -38 }}>
              <div style={{ width: 88, height: 88, borderRadius: 99, background: '#A9B9F2', color: '#28409E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 34, fontWeight: 800, border: '4px solid var(--surface)', flex: 'none' }}>N</div>
              <div style={{ flex: 1, minWidth: 0, paddingBottom: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-.4px' }}>{PD.name}</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, color: '#28409E', background: '#E8ECFB', padding: '4px 11px', borderRadius: 99 }}>👑 Owner</span>
                </div>
              ...
              {profileStats.map((st, i) => (
                <div key={i} style={{ padding: '14px 20px', borderRight: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 11 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--placeholder)', marginTop: 3 }}>{st.label}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {profileTabs.map((t) => (
              <Hover
                ...
                as="button"
                onClick={t.onSelect}
                style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${t.bd}`, background: t.bg, color: t.fg, borderRadius: 99, padding: '9px 18px', font: 'inherit', fontSize: 13, fontWeight: t.weight, cursor: 'pointer' }}
          ...
          {s.profileTab === 'profile' && (
            <>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px', marginBottom: 18 }}>
                <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px', marginBottom: 4 }}>Giới thiệu</div>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, overflow: 'hidden' }}>
                <div style={{ padding: '16px 22px 12px', fontSize: 15, fontWeight: 800, letterSpacing: '-.2px' }}>Thông tin liên hệ</div>
                {profileRows.map((r, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 22px', borderTop: '1px solid var(--line)' }}>
                    ...
                    <span style={{ fontSize: 12.5, color: 'var(--placeholder)', width: 130, flex: 'none' }}>{r.label}</span>
          ...
          {s.profileTab === 'security' && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '18px 22px' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>🔑 Mật khẩu</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>Đổi lần cuối 3 tháng trước</div>
                </div>
                <Hover
                  as="button"
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '18px 22px', borderTop: '1px solid var(--line)' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>🛡 Xác thực 2 lớp (2FA)</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>
                    Bảo vệ tài khoản bằng mã OTP · <b style={{ color: 'var(--ink)' }}>{twoFALabel}</b>
                  </div>
                </div>
                <button
                  onClick={s.toggle2FA}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '18px 22px', borderTop: '1px solid var(--line)' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>📧 Email khôi phục</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>giang.backup@gmail.com</div>
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#0A7B52', background: '#E2F3EC', padding: '5px 12px', borderRadius: 99 }}>Đã xác minh</span>
              </div>
            </div>
          )}
          ...
          {s.profileTab === 'sessions' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ fontSize: 12.5, color: 'var(--ink-2)' }}>Các thiết bị đang đăng nhập vào tài khoản của bạn</div>
                <Hover
                  as="button"
                  ...
                  hover={{ background: '#FBEAE7', borderColor: 'var(--danger)' }}
                >Đăng xuất tất cả thiết bị khác</Hover>
              </div>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, overflow: 'hidden' }}>
                {profSessions.map((x) => (
                  <div key={x.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '15px 22px', borderTop: '1px solid var(--line)' }}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {x.current && (
                          <span style={{ fontSize: 10, fontWeight: 700, color: '#0A7B52', background: '#E2F3EC', padding: '2px 8px', borderRadius: 99 }}>Thiết bị này</span>
                        )}
                      </div>
        ...
        >
          ...
          >
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--line)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-.3px' }}>Chỉnh sửa hồ sơ</div>
            </div>
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '60vh', overflow: 'auto' }}>
              {/* name */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Họ và tên</div>
                <input
              ...
              {/* email */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Email</div>
                <input
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Số điện thoại</div>
                  <input
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Khu vực</div>
                  <input
              ...
              {/* title */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Chức danh</div>
                <input
              ...
              {/* bio */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Giới thiệu</div>
                <textarea
      ...
      {s.overlay === 'changePw' && (
        ...
        >
          ...
          >
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--line)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-.3px' }}>Đổi mật khẩu</div>
              <div style={{ fontSize: 12.5, color: 'var(--ink-2)', marginTop: 3 }}>Mật khẩu mới tối thiểu 8 ký tự</div>
            </div>
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* current password */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Mật khẩu hiện tại</div>
                <input
                  type="password"
                  value={s.pwForm.cur}
              ...
              {/* new password */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Mật khẩu mới</div>
                <input
                  type="password"
                  value={s.pwForm.next}
              ...
              {/* confirm password */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.4px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Xác nhận mật khẩu mới</div>
                <input
                  type="password"
        ...
        >
          ...
          >
            <div style={{ padding: '24px 24px 18px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-.3px' }}>Đăng xuất khỏi AgentAIOS?</div>
              <div style={{ fontSize: 13, color: 'var(--ink-2)', marginTop: 6, lineHeight: 1.5 }}>Bạn sẽ cần đăng nhập lại để tiếp tục sử dụng workspace.</div>
            </div>
