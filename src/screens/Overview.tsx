...
export function Overview() {
  ...
  const onRefresh = () => {
    ...
    Promise.all([s.hydrate(), api.get('/overview').then(setOv).catch(() => {})])
      .then(() => s.fireToast(t('Đã làm mới dữ liệu')))
      .catch(() => s.fireToast(t('Làm mới thất bại')))
  ...
  const userName = (s.profileData.name || '').trim() || t('bạn')
  const ovGreeting = (hr < 11 ? t('Chào buổi sáng') : hr < 14 ? t('Chào buổi trưa') : hr < 18 ? t('Chào buổi chiều') : t('Chào buổi tối')) + ', ' + userName
  ...
  const ovKpis = [
    { icon: '🤖', label: 'Agents', value: String(s.agentsData.length), hasDelta: false, delta: '', deltaUp: true, sub: t('tổng agent'), onOpen: (() => s.setView('agents')) as (() => void) | null },
    { icon: '#', label: 'Channels', value: channelsCount + '', hasDelta: false, delta: '', deltaUp: true, sub: 'public · private · DM', onOpen: null as (() => void) | null },
    { icon: '💬', label: t('Tin nhắn'), value: (ov?.kpis?.messages ?? 0).toLocaleString('vi-VN'), hasDelta: false, delta: '', deltaUp: true, sub: t('tổng trong workspace'), onOpen: null as (() => void) | null },
    { icon: '🗂', label: t('Tasks đang mở'), value: openTasksN + '', hasDelta: true, delta: '-3', deltaUp: false, sub: t('so với hôm qua'), onOpen: (() => s.set({ overlay: 'openTasks' })) as (() => void) | null },
  ].map((k) => ({
    ...k,
  ...
  const DOW = [t('CN'), t('T2'), t('T3'), t('T4'), t('T5'), t('T6'), t('T7')]
  ...
  const ovHealth = [
    ...
    { label: 'Knowledge base', status: 'Healthy', detail: s.knowledgeData.length + ' entries · 0 ' + t('chờ duyệt'), ok: true },
    { label: 'Agent workflow', status: wfRunning + ' ' + t('đang chạy'), detail: s.workflows.length + ' pipeline · 98% success', ok: true },
    { label: t('Kết nối máy chủ'), status: 'Online', detail: '9Router · Tailscale', ok: true },
  ].map((h) => ({
    ...h,
  ...
  const ovWfMini = s.workflows.filter((w) => w.enabled).slice(0, 3).map((w) => {
    ...
    return {
      ...
      label: done + '/' + w.steps.length + ' ' + t('bước'),
  ...
  const openTasksList = s.tasksData.filter((t) => t.status !== 'done').map((t: TaskItem) => {
    const prioMap: Record<string, { fg: string; bg: string; l: string }> = {
      ...
      low: { fg: '#5A6B64', bg: '#EEF2F0', l: t('Thấp') },
    }
    const stMap: Record<string, { l: string; fg: string; bg: string }> = {
      queued: { l: t('Chờ'), fg: '#5A6B64', bg: '#EEF2F0' },
  ...
  const exportReport = () => {
    ...
    rows.push('')
    rows.push([esc(t('Chỉ số')), esc(t('Giá trị')), esc(t('Thay đổi'))].join(','))
    ovKpis.forEach((k) => rows.push([esc(k.label), esc(k.value), esc(k.hasDelta ? `${k.deltaArrow} ${k.delta}` : '')].join(',')))
    rows.push('')
    rows.push([esc(t('Ngày')), esc(t('Tin nhắn')), esc(t('Lượt chạy workflow'))].join(','))
    ...
    s.fireToast(t('Đã xuất báo cáo CSV'))
  }
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <header style={{ flex: 'none', background: 'var(--surface)', borderBottom: '1px solid var(--line)', padding: '16px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginBottom: 3 }}>Workspace › {t('Tổng quan')}</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <span style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-.4px' }}>{ovGreeting} 👋</span>
            <span style={{ fontSize: 12, color: 'var(--ink-2)' }}>{t('Hôm nay')} · {ovDate}</span>
          </div>
        </div>
              ...
              <>
                ...
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, zIndex: 21, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, boxShadow: '0 12px 32px rgba(15,30,25,.16)', padding: 7, minWidth: 232 }}>
                  ...
                  <div style={{ padding: '4px 8px 6px' }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.5px', color: 'var(--placeholder)', marginBottom: 8 }}>{t('TÙY CHỌN KHOẢNG')}</div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '22px 28px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.55fr 1fr', gap: 20, alignItems: 'start', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px' }}>{t('Hoạt động')} · {rangeLabel}</div>
                  <div style={{ fontSize: 11.5, color: 'var(--ink-2)', marginTop: 2 }}>{t('Tin nhắn')} &amp; {t('lượt chạy workflow theo ngày')}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--ink-2)' }}>
                    <span style={{ width: 10, height: 10, borderRadius: 3, background: 'var(--jade)', display: 'inline-block' }}></span>{t('Tin nhắn')}
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--ink-2)' }}>
                    <span style={{ width: 10, height: 10, borderRadius: 3, background: '#A9B9F2', display: 'inline-block' }}></span>{t('Lượt chạy')}
                  </span>
                </div>
              </div>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px' }}>
              <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px', marginBottom: 14 }}>{t('Workflow đang bật')}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {/* top agents */}
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px' }}>
              <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px', marginBottom: 4 }}>{t('Top agents tuần này')}</div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-2)', marginBottom: 16 }}>{t('Theo số task')} &amp; {t('lượt chạy hoàn tất')}</div>
            ...
            {/* system health */}
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px' }}>
              <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px', marginBottom: 14 }}>{t('Sức khỏe hệ thống')}</div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 22px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-.2px' }}>{t('Hoạt động gần đây')}</div>
            <Hover as="button"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 28px' }}>
            ...
            {ovFeedAll.length === 0 && (
              <div style={{ gridColumn: '1 / -1', padding: '18px 0', textAlign: 'center', fontSize: 13, color: 'var(--placeholder)' }}>{t('Chưa có hoạt động gần đây.')}</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div onClick={s.closeOverlay} style={{ position: 'fixed', inset: 0, background: 'rgba(22,32,28,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, animation: 'fadeIn .15s ease' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: 520, maxWidth: '94vw', maxHeight: '82vh', background: 'var(--surface)', borderRadius: 22, boxShadow: '0 24px 60px rgba(22,32,28,.22)', animation: 'pop .2s ease both', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 22px 14px' }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: '-.3px' }}>{t('Tasks đang mở')} · {openTasksCount}</div>
                <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>{t('Các tác vụ chưa hoàn tất trong workspace')}</div>
              </div>
