      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 460, maxWidth: '100%', background: 'var(--surface)', borderRadius: 22, boxShadow: '0 24px 60px rgba(0,0,0,.28)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 24px 4px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: '-.3px' }}>Mời người dùng</div>
              <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>Gửi lời mời qua email để tham gia workspace.</div>
            </div>
          </div>
        ...
        <div style={{ padding: '18px 24px 4px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Email người được mời</label>
            <input
              value={s.inviteForm.email}
              onChange={(e) => s.onInviteEmail(e.target.value)}
              placeholder="vd. ten@zytech.vn"
              autoFocus
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Vai trò khi tham gia</label>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginTop: 9, lineHeight: 1.5 }}>Người được mời sẽ nhận email kèm liên kết tham gia. Bạn có thể đổi vai trò sau khi họ chấp nhận.</div>
          </div>
        </div>
        <div style={{ padding: '18px 24px 22px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <Hover as="button" onClick={s.closeOverlay} style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink-2)', background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: 99, padding: '11px 22px', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ borderColor: 'var(--jade)', color: 'var(--jade-deep)' }}>Hủy</Hover>
          <Hover as="button" onClick={s.submitInvite} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13.5, fontWeight: 700, color: '#fff', background: 'var(--jade)', border: 'none', borderRadius: 99, padding: '11px 24px', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ background: 'var(--jade-deep)' }}>✉️ Gửi lời mời</Hover>
        </div>
      </div>
    </div>
  )
...
function EditUserModal() {
  ...
  const statusBtns = (['active', 'suspended'] as const).map((k) => {
    ...
    const lbl = k === 'active' ? 'Hoạt động' : 'Tạm khóa'
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 440, maxWidth: '100%', background: 'var(--surface)', borderRadius: 20, boxShadow: '0 24px 60px rgba(0,0,0,.28)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 22px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div>
              <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-.3px' }}>Sửa hồ sơ thành viên</div>
              <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 2 }}>Cập nhật thông tin, vai trò và trạng thái.</div>
            </div>
          </div>
        ...
        <div style={{ padding: '6px 22px 4px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Họ và tên</label>
            <input
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Vai trò</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {roleBtns.map((b) => (
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: 'var(--placeholder)', marginBottom: 7 }}>Trạng thái</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {statusBtns.map((b) => (
        ...
        <div style={{ padding: '18px 22px 20px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <Hover as="button" onClick={s.closeEditUser} style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink-2)', background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: 99, padding: '11px 22px', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ borderColor: 'var(--jade)', color: 'var(--jade-deep)' }}>Hủy</Hover>
          <Hover as="button" onClick={s.saveEditUser} style={{ fontSize: 13.5, fontWeight: 700, color: '#fff', background: 'var(--jade)', border: 'none', borderRadius: 99, padding: '11px 24px', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ background: 'var(--jade-deep)' }}>Lưu thay đổi</Hover>
        </div>
      </div>
    </div>
  )
...
export function Users() {
  ...
  // stat cards
  const usersStats = [
    { icon: '👥', label: 'Tổng người dùng', value: String(U.length),  sub: IV.length + ' lời mời chờ' },
    { icon: '🟢', label: 'Đang hoạt động',  value: String(U.filter((u: UserRow) => u.status === 'active').length), sub: 'tài khoản active' },
    { icon: '🆕', label: 'Chờ duyệt',       value: String(SG.length), sub: 'yêu cầu đăng ký mới' },
    { icon: '🛡', label: 'Vai trò',          value: '4',               sub: 'Owner · Lead · Staff · Viewer' },
  ]

  // tabs
  const tabsDef = [
    { key: 'members', label: 'Thành viên', count: U.length },
    { key: 'invites', label: 'Lời mời',    count: IV.length },
    { key: 'signups', label: 'Đăng ký mới',count: SG.length },
  ]
  ...
  const uRoleFilterDefs = [
    { key: 'all',    label: 'Tất cả', count: U.length },
  ...
  const usersRows = U
    ...
    .map((u: UserRow) => {
      ...
      return {
        ...
        statusIcon: susp ? '🔓' : '🔒',
        statusAction: susp ? 'Mở khóa tài khoản' : 'Tạm khóa tài khoản',
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <header style={{ flex: 'none', background: 'var(--surface)', borderBottom: '1px solid var(--line)', padding: '16px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginBottom: 3 }}>Workspace › Người dùng &amp; vai trò</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <span style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-.4px' }}>Người dùng &amp; vai trò</span>
            <span style={{ fontSize: 12, color: 'var(--ink-2)' }}>Quản lý thành viên và phân vai trò</span>
          </div>
        </div>
        <Hover as="button" onClick={s.openInvite} style={{ display: 'flex', alignItems: 'center', gap: 7, border: 'none', background: 'var(--jade)', color: '#fff', borderRadius: 99, padding: '10px 18px', font: 'inherit', fontSize: 13, fontWeight: 600, cursor: 'pointer', boxShadow: '0 6px 16px rgba(10,92,72,.2)' }} hover={{ background: 'var(--jade-deep)' }}>✉️ Mời người dùng</Hover>
      </header>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: '1px solid var(--line)', background: 'var(--surface)', borderRadius: 11, padding: '9px 14px', width: 240, flex: 'none' }}>
                ...
                <input value={s.usersQuery} onChange={(e) => s.set({ usersQuery: e.target.value })} placeholder="Tìm theo tên hoặc email…" style={{ flex: 1, border: 'none', outline: 'none', fontFamily: 'inherit', fontSize: 12.5, background: 'transparent', color: 'var(--ink)' }} />
              </div>
            </div>
                ...
                <Hover key={u.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(240px,1.8fr) 160px 150px 130px 60px', alignItems: 'center', padding: '13px 20px', borderBottom: '1px solid var(--line)' }} hover={{ background: 'var(--bg)' }}>
                  ...
                  <div>
                    {u.isOwner ? (
                      ...
                      <Hover as="button" onClick={u.onCycleRole} title="Đổi vai trò" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: u.roleFg, background: u.roleBg, border: 'none', padding: '4px 11px', borderRadius: 99, cursor: 'pointer', fontFamily: 'inherit' }} hover={{}}>
                  ...
                  <div style={{ textAlign: 'right', position: 'relative' }}>
                    <Hover as="button" onClick={u.onOpenMenu} title="Tùy chọn"
                    ...
                    {u.menuOpen && (
                      <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, zIndex: 30, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, boxShadow: '0 12px 32px rgba(15,30,25,.16)', padding: 7, minWidth: 192, textAlign: 'left' }}>
                        <Hover as="button" onClick={u.onEdit} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, background: 'transparent', border: 'none', borderRadius: 9, padding: '9px 10px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }} hover={{ background: 'var(--jade-soft)', color: 'var(--jade-deep)' }}>
                          <span style={{ fontSize: 14 }}>✏️</span>Sửa hồ sơ
                        </Hover>
                        {u.canSuspend && (
                        ...
                        {u.canRemove && (
                          <>
                            ...
                            <Hover as="button" onClick={u.onRemove} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, background: 'transparent', border: 'none', borderRadius: 9, padding: '9px 10px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'var(--danger)' }} hover={{ background: '#FBEAE7' }}>
                              <span style={{ fontSize: 14 }}>🗑</span>Xóa khỏi workspace
                            </Hover>
                          </>
                        )}
                        {u.isOwner && (
                          <div style={{ padding: '7px 10px', fontSize: 11, color: 'var(--placeholder)', lineHeight: 1.4 }}>Chủ workspace không thể bị khóa hoặc gỡ.</div>
                        )}
                      </div>
                    )}
                  </div>
            ...
            {inviteRows.map((iv) => (
              <Hover key={iv.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '15px 20px', borderBottom: '1px solid var(--line)' }} hover={{ background: 'var(--bg)' }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginTop: 1 }}>Mời bởi {iv.by} · {iv.time}</div>
                </div>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: '#9A6A1B', background: '#FBF1DE', padding: '4px 11px', borderRadius: 99 }}>Đang chờ</span>
                <Hover as="button" onClick={() => s.resendInvite(iv.id)} style={{ fontSize: 12, fontWeight: 600, color: 'var(--jade-deep)', background: 'var(--jade-soft)', border: 'none', borderRadius: 99, padding: '6px 13px', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ background: 'var(--jade)', color: '#fff' }}>Gửi lại</Hover>
                <Hover as="button" onClick={() => s.cancelInvite(iv.id)} style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink-2)', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }} hover={{ color: 'var(--danger)' }}>Hủy</Hover>
              </Hover>
            ))}
            {inviteRows.length === 0 && (
              <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--placeholder)', fontSize: 13 }}>Không có lời mời nào đang chờ.</div>
            )}
          </div>
        )}
              ...
              {signupRows.map((r) => (
                <Hover key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '15px 20px', borderBottom: '1px solid var(--line)' }} hover={{ background: 'var(--bg)' }}>
                  ...
                  <div style={{ minWidth: 0, flex: 1, position: 'relative' }}>
                    <button onClick={r.onToggleMenu} title="Đổi vai trò đề xuất" style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'inherit', maxWidth: '100%' }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--placeholder)', marginTop: 1 }}>{r.email} · qua {r.via} · {r.time}</div>
                    {r.menuOpen && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: 8, zIndex: 30, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, boxShadow: '0 12px 32px rgba(15,30,25,.16)', padding: 7, minWidth: 188 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.6px', textTransform: 'uppercase', color: 'var(--placeholder)', padding: '5px 10px 7px' }}>Vai trò đề xuất</div>
                        {r.roleOptions.map((o) => (
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: r.roleFg, background: r.roleBg, padding: '4px 11px', borderRadius: 99 }}>Đề xuất: {r.roleLabel}</span>
                  <Hover as="button" onClick={r.onApprove}
                    ...
                    hover={{ filter: 'brightness(.94)' }}>✓ Duyệt</Hover>
                  <Hover as="button" onClick={r.onReject}
                    ...
                    hover={{ borderColor: 'var(--danger)', background: '#FBEAE7' }}>✕ Từ chối</Hover>
                </Hover>
              ))}
              {signupRows.length === 0 && (
                <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--placeholder)', fontSize: 13 }}>Không còn yêu cầu nào đang chờ duyệt.</div>
              )}
            </div>
          </>
        )}
